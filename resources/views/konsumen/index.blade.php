...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl border border-secondary-100 shadow-sm overflow-hidden flex flex-col">
        ...
        <div class="p-4 border-b border-secondary-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-secondary-50/50">
            <div class="flex items-center gap-2">
                <div class="relative w-full sm:w-64">
                    ...
                    <input type="text" id="search-konsumen" class="w-full pl-9 pr-4 py-2 bg-white border border-secondary-200 rounded-lg text-sm focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors outline-none" placeholder="Cari nama, pic...">
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table id="table-konsumen" class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-secondary-100">
                    ...
                    <tr class="table-row-hover konsumen-row" data-alamat="{{ $konsumen->nama_konsumen }}">
                        <td class="px-6 py-4 font-mono text-sm text-secondary-600">{{ $konsumen->kode_konsumen }}</td>
                        <td class="px-6 py-4 font-medium text-secondary-900">{{ $konsumen->nama_konsumen }}</td>
                        <td class="px-6 py-4 text-sm text-secondary-700">{{ $konsumen->nama_pic_konsumen }}</td>
                        <td class="px-6 py-4 text-right space-x-1">
                            <button onclick="handleDetail('{{ $konsumen->kode_konsumen }}', '{{ addslashes($konsumen->nama_konsumen) }}', '{{ addslashes($konsumen->nama_pic_konsumen) }}', '{{ $konsumen->created_at->format('d M Y') }}')" class="inline-flex items-center justify-center p-2 text-secondary-400 hover:text-info-600 hover:bg-info-50 rounded-lg transition-colors" data-tooltip="Detail">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </button>
                            <button onclick="handleEdit('{{ $konsumen->kode_konsumen }}', '{{ addslashes($konsumen->nama_konsumen) }}', '{{ addslashes($konsumen->nama_pic_konsumen) }}')" class="inline-flex items-center justify-center p-2 text-secondary-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" data-tooltip="Edit">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                            </button>
                            <button onclick="handleDelete('{{ $konsumen->kode_konsumen }}', '{{ addslashes($konsumen->nama_konsumen) }}')" class="inline-flex items-center justify-center p-2 text-secondary-400 hover:text-danger-600 hover:bg-danger-50 rounded-lg transition-colors">
                    ...
                    <tr id="no-matching-row" style="display: none;">
                        <td colspan="4" class="px-6 py-10 text-center text-secondary-500">
                            Tidak ada data konsumen cocok dengan filter.
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="px-6 py-4 border-t border-secondary-100 flex items-center justify-between">
            <p class="text-sm text-secondary-500">Menampilkan <span id="table-konsumen-start" class="font-medium text-secondary-900">{{ count($konsumens) > 0 ? 1 : 0 }}</span> sampai <span id="table-konsumen-end" class="font-medium text-secondary-900">{{ count($konsumens) }}</span> dari <span id="table-konsumen-count" class="font-medium text-secondary-900">{{ count($konsumens) }}</span> data</p>
        </div>
    </div>
</div>

<x-modal id="modal-form" title="Form Konsumen" maxWidth="max-w-md">
    <form class="space-y-6" method="POST" action="/konsumen">
        @csrf
...
{{-- Modal Detail --}}
<x-modal id="modal-detail" title="Detail Konsumen" maxWidth="max-w-md">
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-y-4 text-sm">
            <div>
                <p class="text-secondary-500 mb-1">Nama PIC Konsumen</p>
...
<script>
    ...
    function handleAdd() {
        ...
        form.reset();
        form.action = '/konsumen?role=' + getRoleQueryParam();
        document.getElementById('form-method').value = 'POST';
        document.querySelector('#modal-form h3').textContent = 'Tambah Konsumen';
    ...
    function handleEdit(kode_konsumen, nama_konsumen, nama_pic_konsumen) {
        const form = document.querySelector('#modal-form form');
        form.reset();
        form.action = '/konsumen/' + kode_konsumen + '?role=' + getRoleQueryParam();
        document.getElementById('form-method').value = 'PUT';
        document.querySelector('#modal-form h3').textContent = 'Edit Konsumen';
        
        populateEditModal('modal-form', {
            nama_konsumen, nama_pic_konsumen
        });
        
        openModal('modal-form');
    }

    function handleDetail(kode, nama, pic, created) {
        ...
        document.getElementById('modal-detail-pic').textContent = pic;
    ...
    function handleDelete(id, name) {
        confirmDelete(name, function() {
            ...
            form.action = '/konsumen/' + id + '?role=' + getRoleQueryParam();
            form.submit();
        });
    }

    document.addEventListener('DOMContentLoaded', function() {
        const searchInput = document.getElementById('search-konsumen');
        ...
        function filterTable() {
            ...
            const rows = document.querySelectorAll('.konsumen-row');
            let visibleCount = 0;
            ...
            const startEl = document.getElementById('table-konsumen-start');
            const endEl = document.getElementById('table-konsumen-end');
            const countEl = document.getElementById('table-konsumen-count');
